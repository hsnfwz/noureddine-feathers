...
export async function POST({ request }: any) {
  try {
    const { profileId, products } = await request.json();

    const productPriceIds: number[] = products.map((product: any) => product.productPriceId);
    ...
    while (i < products.length) {
      ...
      const productPrice: any = productPrices.find((productPrice : any) => productPrice.id === product.productPriceId);
      ...
      shippingCost = shippingCost + (productPrice.shipping_fee * product.quantity);
